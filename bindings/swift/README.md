# Vinary Tree Swift interop binding
...
| Support tier | Tier 2 |
| Distribution | SwiftPM product `VinaryTreeInterop` |
...
| Canonical facade source | [`vinary-tree-interop/bindings/swift`](.) |
...
[`DictionaryResource.swift`](vinary-tree-interop/Sources/VinaryTreeInterop/DictionaryResource.swift)
is the public Swift facade. The root `Package.swift` compiles it with the
canonical C header as a system-library target:
...
Close long-lived resource handles explicitly and use lexical `defer`; `deinit` is a safety net whose timing is not an API guarantee. Close every entries cursor, and release its current generation before advancing or closing it.
