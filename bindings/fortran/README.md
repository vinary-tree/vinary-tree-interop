# Vinary Tree Fortran interop binding
...
| Support tier | Tier 2 |
| Distribution | fpm package `vinary-tree-interop` |
...
`4.0.0-rc.6` train this manifest therefore builds as a `4.0.0` source
...
[`src/vinary_tree_interop.f90`](src/vinary_tree_interop.f90) is the public fpm
module. Compile its ISO C binding declarations with:

```sh
fpm test -C bindings/fortran
```

Project-specific constructors live in their own fpm packages; this neutral
...
Call each derived handle's `close` method; final procedures are exceptional-path protection rather than deterministic scheduling. Close every entries cursor, and release its current generation before advancing or closing it.
