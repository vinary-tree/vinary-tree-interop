...
| Support tier | Tier 2 |
| Distribution | Go module `github.com/vinary-tree/vinary-tree-interop/bindings/go/v4` at tag `bindings/go/v4.0.0-rc.6` |
...
Call `Close` with `defer` immediately after successful construction. Finalizers report leaks but are not a prompt release mechanism. Close every entries cursor, and release its current generation before advancing or closing it.
