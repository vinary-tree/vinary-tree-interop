...
3. **Ledger integrity.** The retain/release count of every resource balances
   ...
   double release is memory corruption. Entry-cursor ownership and batch
   leases likewise settle exactly once (§ 5, § 6).
...
| Interop exchange plane | This crate's structs and constants — two-word handles, POD descriptors, and borrowed batch views | Neutral: layouts and constants only; no code to trust or distrust. |
...
Entries-v1 cursors have a narrower rule than resource-level callback gating:
one cursor is exclusive to one call at an instant, and the consumer must
serialize it regardless of the source dictionary's `PARALLEL_REENTRANT`
claim. Distinct cursors are independent. While a reducer callback runs, its
auto-lease is also a re-entry guard: every same-cursor operation, including
`cancel`, returns `BatchInUse`. The reducer returns `End` to stop successfully.
...
| Malformed compact graph | Misaligned slice pointer; count multiplication overflow; out-of-range root or target; out-of-bounds edge range; unsorted labels; zero or forged value cursor | Accept `vt.dict.graph.v1` only from an immutable resource. Before constructing any safe slice-backed representation, validate pointer nullability/alignment and byte-size arithmetic; then validate every node, edge, flag, reserved byte, range, target, label, and cursor. Publish nothing until the entire view passes. Value cursors remain opaque: producers must validate them before dereferencing backend state. | Compact-graph contract, [ABI reference § 6.4](abi-reference.md#optional-compact-snapshot-graph-vtdictgraphv1); consumer decode tests in liblevenshtein `src/bindings.rs` and producer cursor-forgery tests in libdictenstein `src/bindings.rs`. |
| Malformed entries metadata | Unknown unit/value/order discriminant; `BYTES` value domain; nonzero reserved data; absent-field payload not zero | Receive the three discriminants as raw `uint32_t`, accept only entries-v1's declared domains and lexicographic order, require reserved fields and absent optional fields to be zero, and ignore unknown additive flag bits. Do not construct Rust enums first. | Entries contract, [ABI reference § 6.4](abi-reference.md#optional-dictionary-entries-vtdictentryv1); discriminant and zero-default pins in `tests/discriminant_pins.rs`. |
| Malformed entry batch | NULL with nonzero count; misaligned unit arena; count-times-element-size overflow; descriptor range outside an arena; gaps/overlap; illegal scalar; wrong value cardinality; nonzero reserved field; empty `Ok` batch | Validate nullability, alignment, checked arithmetic, canonical packed ranges, unit-domain values, value-domain encoding, reserved fields, nonempty progress, batch limits, and strict cross-batch lexicographic order before exposing any safe slice. Release the lease even when validation fails. | Entries batch contract, [ABI reference § 6.4](abi-reference.md#optional-dictionary-entries-vtdictentryv1). |
| Forged or stale batch generation | Generation zero on `Ok`; repeated/decreasing generation; release with an older token | Require every successful generation to be nonzero and strictly increasing. Call `release_batch` exactly once with the live generation; reject stale/wrong/no-lease releases as `InvalidArgument` without changing cursor state. Never dereference a view after successful release. | Entries lease state machine, [ABI reference § 6.4](abi-reference.md#optional-dictionary-entries-vtdictentryv1). |
| Reducer status or re-entry abuse | Reducer returns 42, recursively closes its cursor, or stores a batch pointer | The provider settles its auto-lease before interpreting the raw status; maps unknown reducer statuses to `InvalidArgument`; refuses every same-cursor re-entry with `BatchInUse`; and expires every pointer on callback return. Consumer reducers may return only known statuses and never retain arena pointers. | Entries reducer contract, [ABI reference § 6.4](abi-reference.md#optional-dictionary-entries-vtdictentryv1). |
| Foreign semiring token confusion | A stale `(slot, generation)` token; a token issued by another operation context with the same `domain_id`; copied words released twice | Bind every safe weight to the exact retained context that issued it, reject cross-context operations before callbacks, clone through `clone_value`, and consume each owned token exactly once through `release_values`. Providers using arenas validate both slot and generation before access and recycle only released slots. | Dynamic semiring context/token contract, [ABI reference § 7.6](abi-reference.md#76-dynamic-semiring-operation-contexts-vtsemiring1). |
| False semiring law or malformed projection | Provider claims total order but returns `INCOMPARABLE`; quantization changes across equal values; probability is negative or NaN | Treat law flags as claims requiring property conformance before algorithm specialization. Validate raw order values, finite/nonnegative projection domains, stable-byte/equality coherence, and closure-bound consistency. Refuse the specialized algorithm on a violated claim; never let the claim weaken memory or termination bounds. | Separate properties capability and hostile-provider requirements in [ABI reference § 7.6](abi-reference.md#76-dynamic-semiring-operation-contexts-vtsemiring1). |
...
| Inflated compact-graph counts | An immutable graph view claims `SIZE_MAX` nodes or edges, or enough elements to overflow byte-size arithmetic | Reject counts whose multiplication overflows or exceeds `isize::MAX` before forming a slice. A conforming in-process provider necessarily owns the advertised readable memory, but consumers must not turn untrusted counts into pointer arithmetic first. Deployment memory limits remain the backstop against a genuinely huge, readable graph. |
| Hostile entry sizes or nontermination | First key exceeds `max_units`; provider returns endless nonempty batches or generations near wrap | Keep per-batch entry/unit/value limits explicit. A conforming provider returns `LimitExceeded` without advance when one entry cannot fit and guarantees a finite language plus eventual sticky `End`; consumers still apply total entry/work budgets to defend against a false claim. Generation wrap is `LimitExceeded`, never reuse. |
| Leaked entry lease or cursor | Error path omits `release_batch`; copied cursor words are closed twice; callback stores a view | Make batch settlement structural around every successful `next_batch` and every reducer invocation. Treat the cursor as move-only, call `close` exactly once after settling any lease, and make successful close zero both words. Borrowed pointers never outlive release/callback return. |
...
| Message-channel flooding | Fault messages sized by the provider | The query-local fault channel stores at most one `BindingError` in an atomic take-once box (`src/bindings.rs`, `Provider::fault`) and the thread-local `LAST_ERROR` holds one message per thread — constant space per active query owner and per thread by construction. |
